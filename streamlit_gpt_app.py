# streamlit_app.py
"""
RBAC Role Mining Dashboard - Full Streamlit App
...
# Configuration
# -----------------------
API_BASE_URL = "http://localhost:8000/api/v1"  # update if needed
...
def get_clusters():
    try:
        resp = requests.get(f"{API_BASE_URL}/clusters/", timeout=6)
...
def generate_multiple_roles(cluster_id, force=False):
    """
    POST to roles/generate-multiple. If force==True you could pass a param to force regen.
    """
    try:
        payload = {"cluster_id": cluster_id}
        if force:
            payload["force"] = True
        resp = requests.post(f"{API_BASE_URL}/roles/generate-multiple", json=payload, timeout=30)
        if resp.status_code == 200:
            return resp.json()
    except Exception:
        pass
...
if page == "🏠 Dashboard":
    ...
    if clusters:
        ...
        st.warning("No clusters found. Please upload data to the backend or check API.")
...
# -----------------------
    ...
    clusters = get_clusters()
    if not clusters:
        st.warning("No clusters available. Please upload data first (backend).")
        st.stop()
    ...
    with col_left:
        if st.button("🎯 Generate 3 Role Options", type="primary", use_container_width=True):
            with st.spinner("AI is generating 3 different role naming options..."):
                ...
                if result:
                    # store result keyed by cluster_id
    ...
    if pending:
        ...
        if st.button("🚀 Start Batch Generation", type="primary", use_container_width=True):
            ...
            for i, cid in enumerate(pending):
                ...
                progress.progress(int(((i + 1) / len(pending)) * 100))
...
# -----------------------
    ...
    with fcol2:
        filter_style = st.selectbox("Filter by Selected Style", ["All", "Business-Focused", "Technical-Focused", "Hierarchical-Focused"])
        ...
        with st.expander(exp_label, expanded=False):
            ...
            with left:
                ...
                for opt in role.get("role_options", []):
                    ...
                    options_display.append(f"Option {opt.get('option_number')}: {style_label} - {opt.get('role_name')}")

                # find index for selectbox
                try:
                    current_index = next(i for i, o in enumerate(role.get("role_options", [])) if o.get("option_number") == selected_opt_num)
...
### 📊 How It Works
1. Upload cluster data with user-entitlement mappings to the backend
...
### 🔧 Powered By
- Azure OpenAI or equivalent LLM for role generation (backend)
...
# -----------------------
st.divider()
st.caption("RBAC Role Mining System v1.0 | Three Naming Approaches for Maximum Flexibility")
